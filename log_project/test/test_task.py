...
def test_title_length_validation():
    with pytest.raises(ValueError, match="Title must be between 1 and 100 characters."):
        ProjectTask(project_id=3, title="")

    with pytest.raises(ValueError, match="Title must be between 1 and 100 characters."):
        ProjectTask(project_id=3, title="T" * 101)

def test_status_validation():
    task = ProjectTask(project_id=4, title="Status Test", status="completed")
    assert task.status == "completed"

    with pytest.raises(ValueError, match="Status must be one of"):
        ProjectTask(project_id=4, title="Invalid Status Test", status="unknown")

def test_priority_validation():
    task = ProjectTask(project_id=5, title="Priority Test", priority="high")
    assert task.priority == "high"

    with pytest.raises(ValueError, match="Priority must be one of"):
        ProjectTask(project_id=5, title="Invalid Priority Test", priority="urgent")

def test_due_date_validation():
    task = ProjectTask(project_id=6, title="Due Date Test", due_date=datetime.now() + timedelta(days=1))
    assert task.due_date is not None

    with pytest.raises(ValueError, match="Due date must be in the future."):
        ProjectTask(project_id=6, title="Past Due Date Test", due_date=datetime.now() - timedelta(days=1))
